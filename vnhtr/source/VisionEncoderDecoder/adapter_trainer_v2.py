...
if __name__ == '__main__':
    ...
    trainloader = DataLoader(dataset=traindataset, batch_size=16, generator=torch.Generator(device="cpu"))
    valdataset = CustomDataset(anot[-10000:], config)
    valloader = DataLoader(dataset=valdataset, batch_size=32, generator=torch.Generator(device="cpu"))
    ...
    wandb.init(
      project="VN_HTR",
      name=f"adapter_vision_encoder_decoder",
      config={
            ...
            if True:
                with torch.no_grad():
                    ...
                    if cer_score < best_cer:
                        best_cer = cer_score
                        torch.save(model.module.state_dict(), "VisionEncoderDecoder/weights/cp_adapter_vision_encoder_decoder_v2.pt")
