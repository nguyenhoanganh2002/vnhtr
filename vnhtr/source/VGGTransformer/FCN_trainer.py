...
if __name__ == '__main__':
    ...
    model = FCNTransformer(config)
    model.load_state_dict(torch.load("source/weights/cp_fcn_transformer_augmented_data.pt"))
    model.to(device)
    ...
    trainloader = DataLoader(dataset=traindataset, batch_size=48, shuffle=True, generator=torch.Generator(device="cpu"), num_workers=8)
    valdataset = CustomDataset(anot[-10000:], config)
    valloader = DataLoader(dataset=valdataset, batch_size=48, shuffle=False, generator=torch.Generator(device="cpu"), num_workers=8)
            ...
            if True:
                with torch.no_grad():
                    ...
                    for batch in tqdm(valloader):
                        ...
                        t_preds = infer_transformer_base(img[:, :, :, :max_width], model.to(device), backbone="FCN")
                    ...
                    if cer_score < best_cer:
                        best_cer = cer_score
                        torch.save(model.state_dict(), "source/weights/cp_fcn_transformer_augmented_data.pt")
