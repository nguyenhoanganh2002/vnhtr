...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("source/weights/cp_adapter_vgg_transformer.pt", map_location=device))
    model.to(device)
    ...
    valdataset = CustomDataset(pd.read_csv("test_anot.csv")[500:], config)
    valloader = DataLoader(dataset=valdataset, batch_size=196, shuffle=False, generator=torch.Generator(device="cpu"))
    
    #optimizer
    param1, param2 = param_to_update(model)
    optimizer = torch.optim.Adam([{'params': param1, 'lr': 0.00001},
                                {'params': param2, 'lr': 0.00001}], weight_decay=1e-3)
    ...
    # wandb.init(
    #   project="VNHTR",
    #   name=f"finetune-adapter-vgg-transfomer-v2",
    #   config={
    ...
    for epoch in range(n_epoch):
        ...
        if epoch < 1:
            optimizer = torch.optim.Adam([{'params': param1, 'lr': 0.00005},
                            {'params': param2, 'lr': 0.00005}], weight_decay=1e-3)
        # # elif epoch < 3:
        # #     optimizer = torch.optim.Adam([{'params': param1, 'lr': 0.00001},
            ...
            if True:
                with torch.no_grad():
                    ...
                    if cer_score < best_cer:
                        best_cer = cer_score
                        torch.save(model.state_dict(), "source/weights/finetune_adapter_vgg_transformer_v3.pt")
